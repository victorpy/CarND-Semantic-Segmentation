...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        ...
        for images, labels in get_batches_fn(batch_size):
            #print("second loop train_nn "+str(len(images)))
            x,loss = sess.run([train_op, cross_entropy_loss],feed_dict={input_image: images, correct_label: labels, keep_prob:0.75, learning_rate:0.0001})
            #print(loss)
            if i % 10 == 0:
                print("Epoch {}/{} | ".format(epoch+1, epochs), " Loss: {:.4f} ".format(loss))
